...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Visualize trained PPO agents")
...
def visualize_agent(agent: PPOAgent, env: Environment, vis_path: str, num_episodes: int = 1, fps: int = 30):
    """
...
def visualize_all_environments(args):
    ...
    for env_name in env_list:
        ...
        model_pattern = os.path.join(args.model_dir, f"ppo_{env_name}_*.pt")
        ...
        for i, model_path in enumerate(selected_models):
            ...
            # Visualize agent
            vis_path = os.path.join(args.vis_dir, f"ppo_{env_name}_ep{episode_num}_visualization.gif")
        ...
        if args.combine and len(env_gif_paths) > 1:
            combined_path = os.path.join(args.vis_dir, f"ppo_{env_name}_combined_visualization.gif")
    ...
    if args.combine and len(env_gifs) > 1:
        ...
        if latest_gifs:
            all_combined_path = os.path.join(args.vis_dir, "ppo_all_environments_combined.gif")
