...
def plot_metrics(metrics: Dict[str, Any], env: str, window_size: int = 10, output_dir: str = "plots"):
    ...
    plt.subplot(2, 2, 1)
    # Use episode_timesteps if available, otherwise fall back to episode numbers
    if "episode_timesteps" in metrics and len(metrics["episode_timesteps"]) == len(metrics["rewards"]):
        plt.plot(metrics["episode_timesteps"], metrics["rewards"], alpha=0.3, label="Raw")
        # For smoothed rewards, we need to use the corresponding timesteps
        if len(metrics["rewards"]) > window_size:
            # Use the timesteps that correspond to the smoothed rewards
            smoothed_timesteps = metrics["episode_timesteps"][window_size-1:]
            plt.plot(smoothed_timesteps, smoothed_rewards, label=f"Smoothed (window={window_size})")
        else:
            plt.plot(metrics["episode_timesteps"], smoothed_rewards, label=f"Smoothed (window={window_size})")
        plt.xlabel("Timesteps")
    else:
        # Fall back to episode numbers if episode_timesteps not available
    ...
    plt.subplot(2, 2, 2)
    # Use eval_timesteps if available
    if "eval_timesteps" in metrics and len(metrics["eval_timesteps"]) == len(metrics["eval_rewards"]):
        plt.plot(metrics["eval_timesteps"], metrics["eval_rewards"], 'o-', markersize=4)
        plt.xlabel("Timesteps")
    else:
...
def compare_runs(log_dir: str, env: str, runs: List[str], window_size: int = 10, output_dir: str = "plots"):
    ...
    for run in runs:
        ...
        # Use episode_timesteps if available
        if "episode_timesteps" in metrics and len(metrics["episode_timesteps"]) == len(metrics["rewards"]):
            # For smoothed rewards, we need to use the corresponding timesteps
            if len(metrics["rewards"]) > window_size:
                # Use the timesteps that correspond to the smoothed rewards
                smoothed_timesteps = metrics["episode_timesteps"][window_size-1:]
                plt.plot(smoothed_timesteps, smoothed_rewards, label=run)
            else:
                plt.plot(metrics["episode_timesteps"], smoothed_rewards, label=run)
            use_timesteps = True
        else:
    ...
    plt.xlabel("Timesteps" if use_timesteps else "Episode")
    plt.ylabel("Reward")
    plt.legend()
