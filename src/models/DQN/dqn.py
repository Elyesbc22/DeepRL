...
class DQNAgent:
    ...
    def __init__(
        ...
        Args:
            ...
            epsilon_decay: Rate at which epsilon decays (factor for exponential, steps for linear)
            epsilon_decay_type: Type of epsilon decay ('exponential' or 'linear')
            total_timesteps: Total number of timesteps for linear decay
    ...
    def update(self) -> Dict[str, float]:
        ...
        if self.epsilon_decay_type == "exponential":
            ...
            # Linear decay over total_timesteps
            decay_steps = self.total_timesteps * 0.9  # Decay over 90% of total steps
            remaining_steps = max(0, decay_steps - self.steps)
            self.epsilon = self.epsilon_end + (self.epsilon_start - self.epsilon_end) * (remaining_steps / decay_steps)
