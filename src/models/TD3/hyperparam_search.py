...
def evaluate_configuration(metrics: Dict[str, Any]) -> float:
    ...
    """
    # Use the mean of the last 5 evaluation rewards as the score
    eval_rewards = metrics["eval_rewards"]
    if len(eval_rewards) < 5:
        score = np.mean(eval_rewards)
    else:
        score = np.mean(eval_rewards[-5:])
...
def compare_configurations(results: List[Tuple[Dict[str, Any], float]], args: argparse.Namespace):
    ...
    # Add header
    table_data.append(['Rank', 'Score'] + list(results[0][0].keys()))
    
    # Add data
    for i in range(top_n):
        config, score = results[i]
        table_data.append([f"{i+1}", f"{score:.2f}"] + [str(config[k]) for k in config.keys()])
    ...
    for trial in range(num_trials - 2):  # We already have 2 initial configurations
        if trial > 0:  # Skip for the first iteration as we don't have scores yet
            ...
            for config in configurations:
                x = [param_indices[key][config[key]] / (len(search_space[key]) - 1) for key in search_space.keys()]
                X.append(x)
            X = np.array(X)
            y = np.array(scores)
